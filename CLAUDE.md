...
- [x] **IHTT (Inter-Hemispheric Transfer Time) in emotion classifier** — `compute_ihtt()` measures cross-correlation lag between homologous pairs (AF7<>AF8 frontal, TP9<>TP10 temporal). Typical healthy adult: 5-25 ms. Faster IHTT = better cognitive integration. Wired into `_predict_features()`: IHTT focus boost (5% weight) — faster transfer time increases focus_index. `ihtt` dict in all output paths. Also added to `extract_features_multichannel()`. References: Saron & Davidson (1989), Moes et al. (2007).
...
compute_ihtt(signals, fs)                    # Inter-Hemispheric Transfer Time via cross-correlation (AF7<>AF8, TP9<>TP10)
