...
- [x] **Lucidity Predictor** (0dedbbe): `lucidity-predictor.ts` client lib (evidence-based weighted model: 40% recent lucidity avg, 25% 7-day recall rate, 20% recall streak momentum, 15% presleep intention set; `computeLucidityPrediction` returns `{likelihood 0-100, band, factors[4], recommendation, summary}`; `lucidityBand` classifier (low/moderate/high/optimal); `lucidityTrend` 14-day sparkline data; `BAND_LABEL`/`BAND_COLOR`/`BAND_BG` Tailwind constants); `LucidityPredictorCard` component — large % score with band badge, 14-day sparkline when ≥ 3 data points, 4 factor bars, tonight's technique recommendation; reads intentions from localStorage (zero extra requests), shares dream-analysis cache key; wired first in `dream-patterns.tsx` historical section; 30 tests; suite now 199 files / 2869 tests
- [x] **Dream Archetype Classifier** (3d7e988): `dream-archetype.ts` client lib (10 archetypes: chase_threat/flying/falling/search_quest/transformation/water_emotion/social_conflict/mechanical_work/nature_spiritual/death_rebirth; 3 exported fns: `classifyDreamArchetypes` weighted keyword scoring across symbols×3/themes×2/arc×1 returning per-archetype scores in [0,1] sorted descending; `aggregateArchetypes` sums across dream history with dreamCount + prevalence fields; `dominantArchetype` single-dream top hit; `ARCHETYPE_LABEL`/`ARCHETYPE_ICON`/`ARCHETYPE_DESCRIPTION` constants); `DreamArchetypeCard` component — horizontal bars showing top-5 archetypes with icon, label, prevalence %, dream count, description; shares TanStack Query cache key; wired into `dream-patterns.tsx` historical section; 31 tests; suite now 198 files / 2839 tests
- [x] **Dream Symbol Context Analyzer** (5073bde): `dream-symbol-context.ts` client lib (7 pure fns: `buildSymbolContextMap` normalises symbols to lowercase, aggregates count/avgTsi/avgLucidity/topThemes/topArcs/lastSeen/dreamIds per symbol; `symbolMood` classifies "uplifting"/"dark"/"mixed"/"neutral" from avgLucidity+avgTsi; `MOOD_COLOR`/`MOOD_BG`/`MOOD_LABEL` Tailwind constants; `sortedSymbols` 4-key sort (frequency/recent/darkest/brightest) with configurable limit; `symbolSummary` one-line human-readable summary); `DreamSymbolContextCard` component — Personal Dream Dictionary with 4 sort tabs, expandable `SymbolRow` showing mood badge, theme chips, arc phrase quotes, avgTsi + avgLucidity + lastSeen; shares TanStack Query cache key (zero extra requests); wired into `dream-patterns.tsx` historical section; 31 tests; suite now 197 files / 2808 tests
- [x] **Dream Recall Heatmap** (5bd9adb): `dream-recall.ts` client lib (UTC-safe, 7 pure fns: `buildRecallCalendar` 28-day grid with per-day dream counts, `computeRecallStreak` consecutive-day streak with yesterday fallback, `computeRecallRate` fraction of days with ≥1 dream, `recallWeeklyTrend` Monday-aligned weekly rates, `recallTrendDirection` comparing last 2 weeks + insufficient guard when both weeks empty, `shortDate`, `recallCellClass` 4-level intensity); `DreamRecallHeatmap` component — 28-day calendar heatmap (4×7 grid), flame streak badge, 7d/28d recall rate, trend label, weekly bar chart, legend; shares TanStack Query cache key; wired into `dream-patterns.tsx`; 31 tests; suite now 196 files / 2777 tests
- [x] **Presleep Intention Tracker** (a4c7403): `sleep-intention.ts` client lib (localStorage-based, 9 pure/storage fns: `saveIntention`, `getTodayIntention`, `getIntentionForDate`, `getIntentionHistory`, `persistAlignmentScore`, `scoreAlignment` Jaccard-style stemmed keyword overlap returning [0,1], `alignmentLabel`, `alignmentDescription`, `computeIntentionStats`); `PresleepIntentionCard` component — textarea with random nightly prompts, save/update button, alignment result badge after morning scoring, scrollable history rows, 3-column stats when ≥2 scored; wired into `research-morning.tsx` post-submission: scores intention vs dream text + themes, persists result to localStorage; wired into `dream-patterns.tsx`; 35 tests; suite now 195 files / 2746 tests
- [x] **Emotional Arc Trend Tracker** (e5a539d): `emotional-arc-tracker.ts` client lib (8 pure fns: `scoreArcValence` keyword-based [-1,1] valence, `arcLabel`, `aggregateArcTrend` daily grouping, `extractArcPattern` normalized phrase extraction, `topArcPatterns` frequency ranking, `computeArcSummary` positiveRate/negativeRate); `EmotionalArcTrendCard` component — Recharts line chart of arc valence over time with y=0 reference line, 3-column stat summary (uplifting/mixed/distressing %), recurring arc pattern chips; deduplicates TanStack Query cache with `DreamHistoryCard` (same key); wired before history card in `dream-patterns.tsx`; 37 tests; suite now 194 files / 2711 tests
- [x] **Dream Journal History Browser** (1e12f1b): `dream-history.ts` client lib (9 pure fns: `isNightmare`, `isLucid`, `hasInsight`, `entryQualityBand`, `formatEntryDate`, `formatEntryDatetime`, `truncateDreamText`, `applyFilter`, `searchDreams`, `sortNewest`, `computeStats`); `DreamHistoryCard` component — searchable (dreamText/themes/keyInsight/symbols), filterable (All/Nightmares/Lucid/With Insight), expandable row per entry showing full text + emotional arc + key insight + symbols + stats + datetime; nightmare/lucid visual badges; wired into `dream-patterns.tsx` historical section; 38 tests; total suite now 193 files / 2674 tests
- [x] **Weekly Dream Synthesis**: `POST /api/dream-weekly-synthesis/:userId` — fetches last 7 days of stored dream metadata (themes, emotionalArc, keyInsight, threatSimulationIndex, symbols), builds a structured context without raw dream text (privacy-preserving, token-efficient), calls Claude Haiku to generate a 2-3 paragraph narrative + topThemes list; returns 400 when < 2 dreams; `weekly-synthesis.ts` client lib (6 pure fns: `formatDreamRow`, `buildSynthesisContext`, `extractWeekTopThemes`, `countWeekNightmares`, `extractWeekSymbols`, `buildWeeklySynthesisPrompt`); `WeeklySynthesisCard` component (idle/loading/done/error states, theme chips, narrative with left border, regenerate button); wired as first card in dream-patterns.tsx historical view; 33 tests
- [x] **Dream Quality Score**: `GET /api/dream-quality-trend/:userId?days=14` — per-dream composite score (sleep quality ±20 + lucidity +15 + nightmare penalty −25 + dream recall +10, clamped 0-100), daily averages returned as trend array; `dream-quality-score.ts` lib (6 pure fns: `computeDreamQualityScore`, `aggregateDailyScores`, `qualityBand`, `qualityLabel`, `qualityColorClass`, `qualityBgClass`, `averageScore`); `DreamQualityCard` component (score badge with colour band, 14-day Recharts area sparkline, formula legend); wired into `dream-patterns.tsx` as first historical card; 31 tests
- [x] **Recurring nightmare detection + IRT effectiveness**: `GET /api/nightmare-recurrence/:userId` — queries 50 most-recent dreams + all IRT sessions; returns `recentNightmares` (7d), `olderNightmares` (7-14d), `trend` (improving/stable/worsening/unknown), `lastNightmareDate`, `irtSessionCount`, `lastIrtDate`, `postIrtNightmares` (nightmares after last IRT); `nightmare-recurrence.ts` client lib with 5 pure functions (`computeNightmareTrend`, `trendLabel`, `irtEffectivenessLabel`, `shouldSuggestIrt`, `formatShortDate`); `NightmareRecurrenceCard` component (trend badge, 7d/prior-7d window comparison, IRT effectiveness message, IRT CTA when needed); wired into `dream-patterns.tsx`; 23 tests
