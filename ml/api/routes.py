...
# Use TimescaleDB when DATABASE_URL is configured; fall back to SQLite otherwise
try:
    ...
    _logging.getLogger(__name__).warning("TimescaleHealthDB init failed, using SQLite: %s", _e)
    _health_db = HealthBrainDB()
...
# ═══════════════════════════════════════════════════════════════
#  CONTINUOUS BRAIN TIMELINE  (TimescaleDB continuous aggregates)
# ═══════════════════════════════════════════════════════════════
...
async def brain_timeline(
    ...
    Returns ``{ buckets: [{time, value, dominant_emotion}] }``

    bucket: "1m" | "1h" (default) | "1d"
    metric: focus_index | stress_index | relaxation_idx | flow_score |
            creativity_score | attention_score | valence | arousal |
            alpha | beta | theta | gamma | delta
    """
    ...
    try:
        ...
        with psycopg2.connect(db_url) as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                # metric is whitelisted above — safe to interpolate view/column names
                cur.execute(
                    f"""
                    SELECT
                        bucket                           AS time,
...
async def brain_export(
    ...
    metrics: comma-separated list from _VALID_METRICS
    format:  "csv" | "json"
    """
