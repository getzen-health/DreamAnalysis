...
async def get_motor_accuracy(user_id: str):
...
async def get_motor_stats(user_id: str):
...
async def get_motor_history(user_id: str, last_n: int = 50):
...
async def reset_motor_imagery(user_id: str):
