...
def test_api_multichannel(client):
    payload = {"signals": [make_signal(), make_signal(freq=8.0), make_signal(freq=20.0), make_signal(freq=6.0)], "user_id": "test-user"}
    resp = client.post("/imagined-speech/analyze", json=payload)
    assert resp.status_code == 200
...
def test_api_processed_at(client):
    resp = client.post("/imagined-speech/analyze", json={"signals": [make_signal()], "user_id": "test-user"})
