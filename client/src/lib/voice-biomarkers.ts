 ...
 *
 * Extracts wellness-oriented voice features: jitter, shimmer, pitch (F0),
 * speech rate, spectral tilt, pause analysis, and a composite vocal wellness
 * score. ALL language is wellness-framed — no clinical terminology.
 *
...
export function extractVoiceBiomarkers(
  ...
  //
  // Composed of four 25-point components:
  //   1. Low jitter (vocal stability)        → 25 pts
  //   2. Good pitch variability (expression) → 25 pts
  //   3. Normal speech rate                  → 25 pts
  //   4. Balanced spectral tilt              → 25 pts

  // Component 1: Low jitter → higher score (jitter < 0.02 is excellent, > 0.1 is high)
  const jitterScore = Math.max(0, 25 * (1 - Math.min(1, jitter / 0.1)));

  // Component 2: Good variability — penalize both too steady (< 0.05 CV) and too erratic (> 0.35 CV)
  let variabilityScore: number;
  if (pitchCV < 0.05) {
    variabilityScore = 25 * (pitchCV / 0.05); // Too monotone
  } else if (pitchCV <= 0.25) {
    variabilityScore = 25; // Ideal range
  } else if (pitchCV <= 0.4) {
    variabilityScore = 25 * (1 - (pitchCV - 0.25) / 0.15); // Getting erratic
  } else {
    variabilityScore = 0;
  }
  ...
  if (speechRate < 1) {
    rateScore = 25 * speechRate; // Very slow or silent
  } else if (speechRate <= 5) {
    rateScore = 25; // Normal range
  } else if (speechRate <= 8) {
    rateScore = 25 * (1 - (speechRate - 5) / 3); // Fast
  } else {
    rateScore = 0;
  }
  ...
  if (absTilt < 1) {
    tiltScore = 20; // Too flat
  } else if (absTilt <= 8) {
    tiltScore = 25; // Normal range
  } else if (absTilt <= 15) {
    tiltScore = 25 * (1 - (absTilt - 8) / 7);
  } else {
    tiltScore = 0;
  }

  const wellnessScore = Math.round(
    Math.max(0, Math.min(100, jitterScore + variabilityScore + rateScore + tiltScore)),
  );
